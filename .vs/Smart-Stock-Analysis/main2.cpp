...
string reccomend(double price[], int numdays) {
	//variables
	string decide, trend;
	int netprofit, stable;
	
	//calculating net profit
	netprofit = profitloss(price,numdays);
	//trend
	trend = generaltrend(price, numdays);
	if (netprofit >= 2.0 && trend == "up") {//BUY STOCK BECAUSE ITS IN PROFIT RN AND GENERAL TREND IS UP
		decide = "BUY";
	}
	else if (netprofit <= -4.0 && trend == "down") {//AVOID STOCK BECAUSE ITS IN LOSS RN AND GENERAL TREND IS DOWN
		decide = "AVOID";
	}
	else if(netprofit <= -3.0 && trend == "stable"){//BUY STOCK RN BECAUSE GENERAL TREND IS STABLE, AND IT IS CURRENTLY IN LOSS
													//SO IT WILL DEFINITELY GO UP AND YOULL PROFIT
		decide = "BUY";
	}
	else if(netprofit >= 3.0 && trend == "stable"){//AVOID STOCK RN BECAUSE GENERAL TREND IS STABLE, AND IT IS CURRENTLY IN PROFIT
													//SO IT WILL DEFINITELY GO DOWN AND YOULL LOSE
		decide = "AVOID";
	}
	else {
		decide = "HOLD";
...
int main() {
	...
	double profit_loss[MAX_STOCKS];
	string trendfollow[MAX_STOCKS];
	...
	do{//while loop for program until the user decides to opt out
		...
		switch(choice){
			...
			if (numStocks > 0 && numDays > 0) {
				...
				cout << "\n No data loaded yet. Please load stock data first.\n";
			}
			break;
			...
			if (numStocks > 0 && numDays > 0) {
				// Generate recommendations, profitloss and general trend for each stock
				for (int s = 0; s < numStocks; s++) {
					...
					profit_loss[s] = profitloss(priceCol, numDays);
					trendfollow[s]= generaltrend(priceCol, numDays);
				}
				
				
				
				//outputting stock name with their generated stats
				
				cout<<"\n=\\===\\===\\==STOCK ANALYSIS REPORT==\\===\\===\\===\\=\n";
				for(int x = 0; x<numStocks; x++){
					
					cout<<"====================================================================\n\n";
					cout<<"Stock: "<<stockNames[x]<<":"<<endl<<endl;
					cout<<"=> You should definitely "<<recommendation[x]<<" this stock\n";
					cout<<"=> this stock has a net Profit(+ve)/Loss(-ve) of "<<profit_loss[x]<<"%"<<endl;
					cout<<"=> this stock follows a generally "<<trendfollow[x]<<" trend"<<endl;
					cout<<"\n====================================================================\n";
	...
	}while(choice != 4);//exit if user chooses option 4
